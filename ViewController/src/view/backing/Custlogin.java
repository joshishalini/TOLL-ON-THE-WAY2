...
public class Custlogin {
    ...
    public String login() {
        ...
        try {
                    ...
                    ResultSet rs = stt.executeQuery("select * from user_login where username='"+getIt2().getValue()+"' and pass='"+getIt3().getValue()+"'");
                    if(rs.next()){
                       HttpSession session = (HttpSession) FacesContext.getCurrentInstance().getExternalContext().getSession(true);
                  //     session.putValue("user",getIt2().getValue());
                        session.putValue("id", rs.getString(1));
                        System.out.println("Valueeeeeee login------------->"+"id");
                        return "/faces/screen/master/search.jsf";
                    }
                    else
                    {
    ...
    public String regi() {
        // Add event code here...
        return "faces/screen/master/user_reg.jsf";
    }
    ...
    public void bphase(PhaseEvent phaseEvent) {
        ...
        {             try {
                FacesContext.getCurrentInstance().getExternalContext().redirect(FacesContext.getCurrentInstance().getExternalContext().getRequestContextPath()+"/faces/screen/master/search.jsf");
            }
                      catch(Exception er){}
